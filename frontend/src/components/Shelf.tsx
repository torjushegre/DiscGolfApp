...
import { getShelf, moveDisc } from '../services/discs';
...
function Shelf() {
  ...
  const fetchDiscs = async () => {
    try {
      const { data, error: fetchError } = await getShelf();
  ...
  if (loading) {
    return <div className="text-center py-8">Loading shelf...</div>;
  }
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <h2 className="text-3xl font-bold text-gray-800 mb-6">Disc Shelf</h2>
      {discs.length === 0 ? (
        <div className="text-center py-12 bg-white rounded-lg shadow">
          <p className="text-gray-500 text-lg">Shelf is empty</p>
          <p className="text-gray-400 text-sm mt-2">Add new discs from the Add Disc page</p>
        </div>
      ) : (
